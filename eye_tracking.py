...
def calibrate(eye):
    ...
    try:
        best_threshold, size = min(
            trials.items(), key=lambda x: abs(x[1] - iris_size))
...
def printText(frame, text):
    ...
    cv2.putText(frame, text, (width // 2, 50),
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for face in faces:
            ...
            leftX, leftY, leftEAR = getROI(frame, gray, landmarks, 0)
            rightX, rightY, rightEAR = getROI(frame, gray, landmarks, 1)
            blink = False
